...
class ComputerVisionNode(DTROS):
    def __init__(self, node_name):
        ...
        # Executing right turn (blue)
        self.STATE_TURNING_RIGHT = "turning_right"
        # Executing left turn (green)
        self.STATE_TURNING_LEFT = "turning_left"
    ...
    def camera_callback(self, msg):
        ...
        try:
            ...
            # 3. Color detection
            detected_color, contour_rect, contour_area, distance = self.detect_color_lines(
                processed_img)
            ...
            if self.color_detect_pub.get_num_connections() > 0:
                vis_img = self.visualize_detection(
                    undistorted_img, detected_color, contour_rect)
            ...
            if self.undistorted_pub.get_num_connections() > 0:
                undistorted_msg = self.bridge.cv2_to_compressed_imgmsg(
                    undistorted_img)
    ...
    def detect_color_lines(self, hsv_img):
        ...
        for color_name, color_data in [
            ...
            ("red", (self.red_lower1, self.red_upper1,
    ...
    def visualize_detection(self, img, detected_color, contour_rect):
        ...
        if detected_color:
            ...
            if contour_rect:
                ...
                y += self.roi_top
                cv2.rectangle(vis_img, (x, y), (x+w, y+h),
                              color_rgb[detected_color], 2)
    ...
    def state_machine_loop(self, event):
        ...
        if self.state == self.STATE_SEARCHING:
            ...
            if self.detected_color and self.detected_distance >= self.detection_distance:
                self.log(
                    f"Detected {self.detected_color} line at distance {self.detected_distance:.1f} cm")
